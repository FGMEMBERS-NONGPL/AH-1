...
aircraft.light.new("sim/model/AH-1/lighting/strobe-top", [0.05, 1.00], strobe_switch);
aircraft.light.new("sim/model/AH-1/lighting/strobe-bottom", [0.05, 1.03], strobe_switch);

# beacons ===========================================================
var beacon_switch = props.globals.initNode("controls/lighting/beacon", 1, "BOOL");
aircraft.light.new("sim/model/AH-1/lighting/beacon-top", [0.62, 0.62], beacon_switch);
aircraft.light.new("sim/model/AH-1/lighting/beacon-bottom", [0.63, 0.63], beacon_switch);
...
var nav_lights = props.globals.getNode("sim/model/AH-1/lighting/nav-lights", 1);
...
var fuel = {
	init: func {
		...
		me.pump_capacity = 6.6 * L2GAL / 60; # same pumps for transfer and supply; from AH-1: 6.6 l/min
...
var turbine = props.globals.getNode("sim/model/AH-1/turbine-rpm-pct", 1);
var torque_pct = props.globals.getNode("sim/model/AH-1/torque-pct", 1);
...
var update_torque = func(dt) {
	...
	torque_pct.setDoubleValue(torque_val / 10500);
}



# blade vibration absorber pendulum
var pendulum = props.globals.getNode("/sim/model/AH-1/absorber-angle-deg", 1);
...
var vibration = { # and noise ...
	...
	update: func(dt) {
		...
		} elsif (airspeed > 30) { # Blade Vortex Interaction (BVI)    10 deg, 85 kts max?
...
var Skid = {
	new: func(n) {
		var m = { parents: [Skid] };
		var soundN = props.globals.getNode("sim/model/AH-1/sound", 1).getChild("slide", n, 1);
...
var volume = props.globals.getNode("sim/model/AH-1/sound/volume", 1);
...
var crash = func {
	if (arg[0]) {
		# crash
		setprop("sim/model/AH-1/tail-angle-deg", 35);
		setprop("sim/model/AH-1/shadow", 0);
		...
		# uncrash (for replay)
		setprop("sim/model/AH-1/tail-angle-deg", 0);
		setprop("sim/model/AH-1/shadow", 1);
...
var rotor_step = props.globals.getNode("sim/model/AH-1/rotor-step-deg");
...
#var config_dialog = gui.Dialog.new("/sim/gui/dialogs/AH-1/config/dialog", "Aircraft/AH-1/Dialogs/config.xml");
